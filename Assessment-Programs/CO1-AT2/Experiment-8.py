...
def plot_results():
    ...
    ax = axes[2]
    window = 20
    for cum, label, color in [(cum_random, "Random", "#F44336"), (cum_greedy, "epsilon-greedy", "#2196F3")]:
        diffs = np.diff(cum, prepend=0)
        ma = np.convolve(diffs, np.ones(window) / window, mode="valid")
        ax.plot(range(window - 1, window - 1 + len(ma)), ma, label=label, color=color, linewidth=1.5)
    ax.set_xlabel("Trial")
...
if __name__ == "__main__":
    print("Experiment 8: Random vs epsilon-greedy MAB Comparison\n")
    print(f"Arms: {N_ARMS} | Trials: {N_TRIALS} (fixed time constraint)")
    print(f"True means: {np.round(TRUE_MEANS, 2)}")
    print(f"Best arm: {np.argmax(TRUE_MEANS)} (mu={TRUE_MEANS[np.argmax(TRUE_MEANS)]:.2f})\n")

    final_random, final_greedy = plot_results()

    print(f"\nFinal cumulative rewards:")
    print(f"  Random  (epsilon=1.0): {final_random:.1f}")
    print(f"  epsilon-greedy (epsilon=0.1): {final_greedy:.1f}")
    print(f"  Improvement: {final_greedy - final_random:.1f}")
    print(f"\nConclusion: epsilon-greedy {'outperforms' if final_greedy > final_random else 'is outperformed by'} random selection")
