...
N_ARMS = 10
N_TRIALS = 500
TRUE_MEANS = np.random.randn(N_ARMS)  # true Q*(a) for each arm
...
def plot_results():
    ...
    # Right: average reward (moving window)
    ax = axes[1]
    window = 20
    for eps, color in zip(epsilons, colors):
...
if __name__ == "__main__":
    print("Experiment 2: ε-greedy Multi-Armed Bandit (500-Trial Budget)\n")
    ...
    for eps in [0.1, 0.3, 0.5]:
        ...
        chosen_best = np.sum(np.argmax(np.random.randn(N_TRIALS, N_ARMS), axis=1) == best_arm)
        print(f"  ε={eps}: Final cumulative reward = {cumulative[-1]:.1f} | "
