...
N_ACTIONS = 3  # Reduce, Maintain, Increase
ACTION_NAMES = ["Reduce", "Maintain", "Increase"]
THRESHOLD = 3  # level 3 = 60-80%; staying below means levels 0,1,2

# Typical daily demand pattern (normalized 0-1)
BASE_DEMAND = np.array([
    0.2, 0.15, 0.1, 0.1, 0.15, 0.25,  # 0-5
    0.4, 0.6, 0.7, 0.65, 0.6, 0.55,  # 6-11
...
def step(state, action):
    ...
    # Base demand pushes level up
    demand_push = 1 if BASE_DEMAND[hour] > 0.6 else 0
    new_level = min(new_level + demand_push, 4)

    # Reward with shaping
    if new_level < THRESHOLD:
        reward = 10  # below threshold — good
    elif new_level == THRESHOLD:
        reward = -5  # at threshold — risky
    else:
        reward = -20  # above threshold — bad

    # Comfort cost for reducing
    if action == 0:
        reward -= 2

    # Potential-based reward shaping
    # Potential = how far below threshold we are
...
def simulate_day(Q):
    """Simulate one day with learned policy."""
    levels = []
    hour = 0
...
def plot_results(Q, episode_rewards, compliance):
    ...
    # Bottom-left: simulated day
    ax = axes[1, 0]
    levels, actions = simulate_day(Q)
    level_pct = [(l + 0.5) * 20 for l in levels]  # convert to percentage
